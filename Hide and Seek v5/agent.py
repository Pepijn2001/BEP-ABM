...
class Hider(Agent):
    def __init__(self, unique_id, model, pos, age):
        ...
        self.direction_chosen = False  # nodig voor strategie direction_traveling
        self.direction = None  # direction die gekozen wordt
        self.strategy = "direction_traveling"
        self.end_node = None
    ...
    def move(self):
        if not self.lost:
            ...
            new_nodes_pos = [agent for agent in neighbors if (isinstance(agent, Patch)) and (not agent.tree)]
            new_pos = self.random.choice(new_nodes_pos).pos
        ...
        if self.strategy == "direction_traveling":
            if not self.direction_chosen:
                ...
                while True:
                    ...
                    print("X", new_pos[0])
                    print("Y", new_pos[1])
                    self.end_node = [agent for agent in self.model.grid.get_cell_list_contents([new_pos])
                                     if isinstance(agent, Patch)][0]
                    self.start_node = [agent for agent in self.model.grid.get_cell_list_contents([self.pos])
                                     if isinstance(agent, Patch)][0]
                    if (new_pos[0] == 0) or (new_pos[0] == (self.model.width - 1)) or (new_pos[1] == 0) or (new_pos[1] == (self.model.height - 1)):
                        self.path = self.find_path()
                        print(self.path)
                        self.path_count = 0
                        break
            else:
                if self.pos != self.end_node.pos:
    ...
    def find_path(self):
        ...
        # Open en closed lijsten maken, start- en eindnode waardes aanmaken
        open_list = []
        closed_list = []
